...
- **Reviewers use the skill-first table (user ruling 2026-09-02).** Techlead verdicts go through `superpowers:requesting-code-review` (plugin-namespaced; the bare name does not resolve); auth-tier issues add `security-review`; §7.17 lines go through `infi-lessons`. The first line of every verdict and evidence file names the skills invoked. PMO rejects a verdict that does not.
