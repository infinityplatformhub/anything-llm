...
- Name: phase0-gate.spec.js >> 03 admin logs in
- Location: tests/phase0-gate.spec.js:85:1
...
```yaml
- generic [ref=e3]:
  - img "ApproofWorkspace" [ref=e6]
  - generic [ref=e7]:
    - heading "Welcome" [level=1] [ref=e12]
...
> 87  |   await page.getByLabel(/username/i).fill(ADMIN.username);
  ...
  178 |   execSync("e2e/scripts/up.sh restart-app", { cwd: process.env.E2E_ROOT });
  179 | 
  180 |   let ready = false;
  181 |   for (let i = 0; i < 60 && !ready; i++) {
  182 |     ready = (await request.get("/api/ping").catch(() => null))?.ok();
  183 |     if (!ready) await new Promise((r) => setTimeout(r, 1000));
  184 |   }
  185 |   expect(ready).toBe(true);
  186 | 
  187 |   // Login again with the SAME credentials created before restart.
```