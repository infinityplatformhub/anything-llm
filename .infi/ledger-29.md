...
- `Fix:` **CoreJobWorker.claim spread `job.actor` ทับ actor ที่ resolve สด** — job row ที่เขียนไว้ก่อน revoke (หรือโดย enqueue path ที่ถูกเจาะ) เลือก `workspaceIds`/`orgId`/`impersonatedBy` ของตัวเองตอนรัน · แก้เป็น resolved ชนะเสมอ: row บอกแค่ว่า "รันในนามใคร" ส่วนสิทธิ์ resolve ใหม่ทุก claim — RED-proven
...
- `Note:` seam 07 typedef อัปเดตแล้ว (`orgWide:boolean` + หมายเหตุว่า driver ข้าม workspace narrowing แต่ยังใช้ predicate อื่นครบ)
...
- `Fix:` (RED เจอเอง ไม่อยู่ใน recon) org-wide branch เดิมเช็ค `actor.type` — key เป็น `service` เสมอ ดังนั้น **key ที่ทำงานแทน user จะอ่านเอกสารทั้ง org ทั้งที่ user เจ้าของอ่านไม่ได้** · เปลี่ยนไปเช็ค `grantPrincipal.type`
- `Fix:` (RED เจอเอง) workspace-bound key สืบทอด reach เต็มของ creator — `narrowToKeyBinding` intersect กับ `api_keys.workspaceId` ซึ่ง**แคบลงอย่างเดียว** ไม่มีทางกว้างขึ้น แม้ binding ถูก forge
...
- `Fix:` (RED เจอเอง) **cross-credential grant confusion** — `validBrowserExtensionApiKey` เขียน `apiKeyContext` เหมือนกัน แต่ `keyId` มาจากตาราง `browser_extension_api_keys` คนละ id sequence กับ `api_keys` → extension key id N ได้ grant ของ creator ของ API key id N ซึ่งเป็นคนละคน · แท็ก `keyKind: "browser-extension"` แล้ว resolver ข้าม branch ไปใช้ `locals.user` ที่ extension resolve มาแล้ว · แท็กแทน infer เพื่อไม่ให้ context shape ใหม่หลุดเข้า branch ผิดเงียบ ๆ
...
- `Fix:` resolve workspace **ครั้งเดียว** แชร์สองครึ่ง — ตอนแรกเขียนเป็น 2 lookup แล้ว `mockResolvedValueOnce` ถูกกินโดยครึ่งแรก bound-key test แดง · สองรอบยังเป็นสองโอกาสให้สองครึ่งไม่ตรงกันด้วย
...
- `Note:` **ไม่ได้ปิดรูทั้งหมด** — แคบลงเหลือ "ต้องรู้ session id ที่ออกโดย embed นี้" ส่วน unguessable token (signed cookie / HMAC ตอน session start) เป็น issue แยกตาม PMO ruling · เขียนไว้ที่ call site ด้วย เพื่อไม่ให้คนอ่านเข้าใจว่าปิดครบแล้ว
...
- `Fix:` `DocumentVectors.deleteForWorkspace` มีบั๊กเดียวกัน (recon ระบุแค่ `models/vectors.js:14,47` เป็นของ T-5) — แก้ที่นี่เพราะเป็นเคสที่ guard comment ของ job ระบุชื่อไว้ตรง ๆ
...
- `Fix:` **9 failures ที่ผมรายงานว่าเป็น pre-existing ของ main นั้นผิด** — DB `approofworkspace_t4b` ของผมไม่เคยรัน `migrate deploy` (ไม่มีตาราง `api_keys` ด้วยซ้ำ) · PMO push back ถูก · บทเรียน: อย่า carry failure เป็น pre-existing โดยไม่ verify กับ baseline ที่สะอาดจริง
...
- `Carry → T-5:` filter ต้องส่งเข้า `queryAuthorized` ทุก call site · subscriber `authorization-cache` ต้องลงพร้อม cache wiring (DoD ไม่ใช่ตามหลัง) · `streamResponse` มี `actor` พร้อมใช้แล้วที่ telegram `:226`
- `Carry → T-6:` flip `ENABLE_DOC_VECTORS_CANONICALIZE` หลังย้าย 7 provider · เมื่อ flip แล้ว call site ของ T-4b ไม่ต้องแก้อีก (match ทั้งสอง id อยู่แล้ว)
- `Carry → T-7:` telegram `approved_users` ไม่มี user id (ดู W-11) · embed session id ที่เดาไม่ได้ (ดู W-10) · `chat.read_others` ใน READ_ACTIONS ตอนแยก `content_moderator`
