...
- **§2 enforcement**: option **(a)** — the sweep test lands in #53; the `evaluate()` scope guard (option b) becomes a follow-up issue. Rationale as written: an authority-free action misused at workspace scope is a correctness bug, not a privilege bug, and does not justify changing the decision path in the same issue that introduces the action.
- `org.member` is asked against `orgResource` ONLY, carries no authority, is seeded to every org role, and joins both `BASELINE_GRANTABLE` and `READ_ACTIONS`.
- 5 routes move; the 2 gates in `chat.js` stay on `chat.send` — they mean it.
- DoD 2 (impersonated session still 403 on `POST /workspace/:slug/chat`) and DoD 3 (org-wide member grant + zero memberships → denied `workspace.read`) are required as written.
- `requireSelfSession` is REPLACED by the real action, not kept beside it (#52 residual).
- Order: #52 gate/QA → merge → #50 → #53.
