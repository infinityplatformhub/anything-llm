...
class Milvus extends VectorDatabase {
  ...
  normalize(inputString) {
    ...
    if (new RegExp(/^[a-zA-Z_]/).test(normalized.slice(0, 1)))
      normalized = `approofworkspace_${normalized}`;
    return normalized;
  }

  async connect() {
    // #88: normalised, like every other comparison of a VECTOR_DB value.
    // getVectorDbClass resolves "MILVUS" to this provider (#87), so a raw
    ...
    if (normalizeVectorDbKey(process.env.VECTOR_DB) !== "milvus")
  ...
  async totalVectors() {
    ...
    for (const name of collection_names.filter((n) =>
      n.startsWith("approofworkspace_")
    )) {
