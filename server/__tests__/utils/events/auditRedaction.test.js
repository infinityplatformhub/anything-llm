...
describe("#71: invite codes never reach the audit log", () => {
  ...
  test("API keys and extension keys are redacted too", async () => {
    // Same shape, same risk, and nothing was guarding them either. A pattern
    // covering only the credential that happened to be reported would leave its
    // siblings live — so both sibling generators are asserted with their REAL
    // functions, not with lookalike strings.
    const { ApiKey } = require("../../../models/apiKeys");
    const {
      BrowserExtensionApiKey,
    } = require("../../../models/browserExtensionApiKey");

    for (const secret of [ApiKey.makeSecret(), BrowserExtensionApiKey.makeSecret()]) {
