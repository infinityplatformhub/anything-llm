...
run("HMAC API key PostgreSQL integration", () => {
  ...
  test("stores only digest and returns raw API key once", async () => {
    const { apiKey, error } = await ApiKey.create(null, "integration");
    ...
    expect(await ApiKey.resolve(apiKey.secret)).toMatchObject({ id: apiKey.id, scopes: ["*"] });
  ...
  test("rejects expired and revoked keys and updates last use", async () => {
    ...
    const expired = await ApiKey.create(null, null, { expiresAt: new Date(Date.now() - 1000) });
