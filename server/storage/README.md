...
- Docker Instance
  - Get your ApproofWorkspace docker container id with `docker ps -a`. The container must be running to execute the next commands.
  ...
  - Run `docker container exec -u 0 -t <ANYTHINGLLM DOCKER CONTAINER ID> chown -R approofworkspace:approofworkspace /app/collector /app/server`
