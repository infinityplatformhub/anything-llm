...
- **#132 / QA-3 — a fixture that grants more than the action under test turns capability tests into shape tests.** HOLDS_SYSTEM_READ also set settings.write, so R3/R6 could not tell the right guard from the wrong one; narrowing the fixture to the single action turned three shape tests into capability tests at zero cost (6/6 right, 4 red wrong).
- **#132 / QA-3 — an inherited-untested mark covers the line it sits on, not every copy.** The shared body carries the mark; ManagerRoute's copy (:192) does not and deleting it is green. Say "one of two" in the ledger so RF-L is not read as closing both.
- **#138-perm / Dev1 — two branches that each bump the same literal by one merge cleanly and are both wrong.** vocabulary-diff pin 62→63 on #137 AND on #138-perm; git shows no conflict, the second merge leaves 63 with the suite green on neither. Sequence explicitly: the second to merge re-bumps (64, and POLICY_VERSION_ROWS 13).
- **#138-perm / Dev1 — splitting migration.sql on ';' before stripping comments executes comment fragments as SQL.** Reads as a migration defect; was the replay test's defect. Strip comments first, assert the statement count.
- **V8 / TL-1 — React 18 removed the setState-after-unmount warning, so "assert no warning" cannot fail.** Assert addEventListener/removeEventListener on the same MediaQueryList with the same handler reference, with a mounted control.
- **#141 / TL-1 — a CHECK constraint with per-provider branches means the new provider's branch is the FIRST thing to write.** identity_providers_one_shape is two-branch; a Lark row satisfies neither, so the resolution fixture cannot insert until the third branch exists. Keep NOT NULL and write empty strings (the documented LDAP encoding); do not match on the free-text `provider` key.
- **#138 / QA-1 — "the queue refuses the worker" and "the worker's writes are refused" are different questions with different answers.** Measure them separately; a green RF-2 (LeaseLostError at complete) read as "safe" hides the apply that landed 7 rows for a 6-plan.
- **Sidebar audit / QA-1 — a derived expectation must state how many items came from each source.** With no `action=` call sites on main, a source-only scan yields 0 and "0 missing" reads as success; feed the audit table too, print per-source counts, and exit non-zero on empty extraction.
- **#142 / Dev5 — "Test suite failed to run" has more than one cause; reproduce on a clean tree before naming the mechanism.** The reported SlowBuffer import failure did not reproduce; the real red was an `afterAll` teardown (server close + prisma disconnect + DROP DATABASE) exceeding jest's 5 s hook default — 18/18 pass, suite FAIL.
- **#138 / TL-2 + QA-1 — "the code is right" and "the suite would notice if it were wrong" are different measurements.** QA-1 measured the code (heartbeat refuses after expiry); TL-2 measured the suite (delete the clause, 9/9 green). Both correct; the NIT is a missing fixture, and a probe that agrees with the code proves today, a committed fixture proves tomorrow.
- **#135 / Dev2 — a repository-level fixture green on merged code is not RED for a route issue.** RF-1/2/3/P5 pass by calling offboardUser directly; the only assertions that can be red for #135 are the three route fixtures. Report "5 green (primitive), 0 route fixtures yet", never "RED 5".
- **#135 / Dev2 — when an assertion reds unexpectedly, find out why before changing it.** "All three orphan classes remain" reddened on memberships because group_members HAS an FK with onDelete: Cascade (the other two are TEXT principal_id, no FK). Assert memberships == 0 with a comment so a future cascade removal reds instead of silently adding a third orphan class.
- **#141 / QA-2 — an optional-chained call on a method that does not exist passes on nothing.** `driver.enumerate?.()` fell through to Promise.resolve(); the real method is listPrincipals. And race every await that can hang against a wall-clock deadline that prints a named RED — an unraced hang is indistinguishable from a crashed runner. (macOS has no `timeout(1)`; use a background kill.)
- **V8 / Dev4 — assert the handler REFERENCE on add/remove, not the call count.** A hook that removes a freshly bound function, or cleans up the wrong MediaQueryList, passes a count check and still leaks; identity + mounted control is the only witness. And a non-vacuity guard belongs in the same row as the assertion it guards, or someone satisfies one and not the other.
- **Sidebar audit / Dev4 — an RF over an empty set is vacuously green; assert the set is empty with a floor and say what green means.** RF-P today has no guard naming any of the 10; the test states that, so a green is evidence about scope, never about gating.
- **Sidebar audit / Dev4 vs QA-3 — two oracles can both be right and still disagree when they count different things.** QA-3's 27/25/1/0 counts sidebar ENTRIES visible per role; Dev4's 11/4/3/0 counts ACTIONS held per role. Name the metric beside every number before comparing.
- **#138-perm / TL-1 — set-equality on actions cannot see a column the migration forgot.** Every prior permission migration inserts `category` (and 102000 adds `scope`); this one used `("action","description")` alone, so a fresh install and a migrated instance hold different rows for the same action. Copy the INSERT column list from the most recent precedent, and set `scope: 'org'` for org actions so a miswired workspace ask is a contract error, not a decision.
- **#138-perm / QA-3 — a deny probe's control must be an action the role holds ON THAT BRANCH.** system.read is held by setup_admin only after #137; on a branch without #137 the control was false too and the probe looked like a working deny. Pick a control from the base's own grants.
- **#135 / QA-2 — when the contract forbids a helper in the model, a model-layer probe going GREEN is the failure signal.** P4/P5 red + P6 (route) green is the correct-fix signature; "P5 flipped" means a helper was added. Write the expected red/green per assertion into the runner output before the SHA arrives so the PMO's expectation cannot invert it.
- **RF-P / QA-1 — a second live selector changes the source count; verify a jump before trusting it.** 1 SystemReadRoute + 26 `capability:` entries = 27 sites; reproduced the 1 independently, then split by selector, before accepting 27.
- **#142 / Dev3 — an absolute path to `npx` does not pin the node it runs.** `/opt/homebrew/opt/node@22/bin/npx jest` ran the suite under node 26 (process.version inside jest = v26.7.0). Invoke `<node22>/bin/node ./node_modules/.bin/jest` and print process.version in the log. The wrong interpreter hid a REAL bug (jobs.traceId NOT NULL, `request.id` undefined → every enqueue 500) and manufactured a fake one.
- **#138-perm / QA-3 — a column with `@default("")` fails silently on BOTH the migration and the seed path.** category='' on directory.sync alone out of 63 rows; a DB built by migrate+seed still had it empty, so fixing migration.sql alone leaves seed-only installs wrong. Assert the value on each path separately; set-equality on (action, category, scope) tuples; count(*) WHERE category='' == 0 as non-vacuity.
- **Logging rule — a catch that logs only `error.message` logs a blank line for Prisma errors.** Log name + code + message.
- **#135 / TL-2 — route fixtures that call handlers off `app._router.stack` bypass validatedRequest/requirePermission.** They can assert cleanup, not gating: strip the permission and they stay green. One comment per fixture naming the sweep as the guard's coverage, so a green suite is not read as "routes are gated".
- **#135 / TL-2 — no seeded role holds user.manage without role.revoke, so every refusal fixture must CONSTRUCT that role and match /role\.revoke/**, or a principal refused by an earlier guard satisfies it (F8-vs-F9 shape). Assert the actor by IDENTITY (locals.actor), not shape. Rollback fixture needs ≥2 users or loop-vs-truncate give the same bump count. Assert revoked_by_type as well as id (singleUser stores the literal "single-user").
- **Harness rule / QA-1 — every harness prints its runtime as its first line and launches probes through an absolute node path.** `#!/usr/bin/env node` and `npx` both re-resolve to the machine default; a baseline recorded under the wrong major can be a fabricated red set (#139's setup guard throws, which reads as "suite broken"). QA-1's V8 baseline survived by luck (identical under 22), not by construction.
- **#135 / QA-2 — a "symmetric" fix across routes with different middleware stacks is not symmetric.** validApiKey sets no locals.actor (requirePermission does); the same line at both sites 500s on the API route. Resolve the actor per route (resolveActor carries grantPrincipal so the human behind the key is what the guard sees); never paper it with an exempt principal.
- **#135 / QA-2 — a route that only deletes today becomes a route that revokes grants tomorrow.** DELETE /v1/admin/users/:id accepts a key whose creator lacks role.revoke (200); once offboarding is wired through it, that key revokes. Decide the refusal explicitly; it is not a side effect.
- **#142 / Dev5 — a load-dependent defect cannot be witnessed by "the suite passes".** The untimed afterAll passed 4/4 on an idle machine and failed only under concurrent suites ("too many clients"). Guard it STRUCTURALLY (brace-matched scan naming untimed slow teardowns, with a control proving the matcher finds one), and record the idle-vs-loaded measurement as the justification.
- **#135 / TL-2 — when cleanup is attached to a delete, the delete's guard follows the new effect.** Deleting a user now removes grants; a principal that may not revoke may not delete. Offboard + delete in ONE transaction; refusal = 403 naming role.revoke (never a bare 500); atomic fixture asserts user row + grants + ACL all intact after the 403.
- **PMO gate rule — symlinked node_modules makes wt-bootstrap refuse (§7.6b) and leaves the Prisma client ungenerated; the reds read as PrismaClientInitializationError.** Install or copy, never symlink, in a gate worktree.
- **#138-perm / Dev1 — two deployment shapes (migrate+seed vs db-push+seed) must agree on every column the engine reads.** `seed.js` never wrote `scope`, so seed-only databases had EVERY action `any` — including `org.member` (#53), whose purpose is refusing workspace resources. Fix at the map (ACTION_SCOPES) for all actions, and `update:` on the upsert so an already-seeded database is corrected too. Assert the seed-only shape, not just the migrated one.
- **#142 / QA-2 — a guard test is the only reliable witness for a load-dependent class; say so in the evidence.** The protected suite passes on an idle box with the mutant applied (teardown ~2s vs 5s default). "Revert and X goes red" must name the suite that actually reds. State the scanner's shape coverage (async-arrow afterAll only; afterEach unscanned) next to `expect(offenders).toEqual([])` so it is not read as "none exist".
- **#135 / QA-3 — a 403 the client cannot explain is half a refusal.** `requirePermission` returns `{ error: "Forbidden." }` (no action) and the engine path `sendStatus(403)` (empty body → client JSON-parse error in the toast). Naming the action in the body is a seam ruling, not a per-route fix.
- **#135 / TL-2 (correcting own condition) — a refusal's ATTRIBUTION lives server-side; the client body says what every other route says.** `requirePermission` maps AuthorizationContractError to `sendStatus(500)` (:92), so a refusal raised inside a transaction surfaces as an empty 500 unless the route catches it and answers 403 with a JSON body. Assert status + valid JSON + audit/log naming the action; never assert the action in the body from one route (drift with the seam). Seam inconsistency (:80 JSON vs :83 empty) and `res.ok` unchecked in the frontend are separate issues in separate lanes.
- **#142 / QA-2 — a checker that enumerates ONE syntactic form silently exempts every other form, and the exemption is invisible because the check passes.** Quantify the unscanned shapes (88/2/0/9) and name the real gaps (one) before calling a structural guard "tree-wide". Same drift as #139 .nvmrc.

- **#138 (QA-3): measured one deploy shape, concluded for both.** `migrate deploy + seed` and `db push + seed` produce different rows when migration INSERTs omit columns and seed `upsert` has `update: {}`. Any claim about seeded rows names the shape it was measured on, and schema/seed issues measure both.
- **#post-merge (QA-1): `grep -vc` after a pipe counts non-matching lines of the whole stream, not "matches failing a second filter".** Its failure mode is a plausible number. Filter first, then count, then print the set (see print-what-the-filter-removed).
- **#post-merge (QA-1): `doctor.test.js:403` needs the `vector` extension; `ci.yml:16` runs plain `postgres:16` while QA boxes run `pgvector/pgvector:pg16`.** Pinned image in ci.yml is the one that decides; run it, not the local one, before calling a test green or red.
- **#135 (Dev2): a transaction rewrite that replaces a model call with a raw `tx.<table>.delete` drops every side effect the model call carried** (here `revokedAt` via `revokeCredentialsFor`). New tests all green; only the full suite caught it. Export the side effect and call it inside the tx; run the full suite before declaring, not the new file.
- **#138 (Dev1): an allowlist entry needs two negative controls before it is accepted** — remove the entry, gate must fail on this file; add a second offending file, gate must fail on that one only. Without them "gate passed" and "gate stopped checking" print the same line.
- **#135 (QA-2): an assertion containing `&& true` (or `>= 0`) is not an assertion.** Mutant wiping the rows stayed green. Count before/after and require equal AND non-zero.
- **#135 (QA-2): a status-code oracle across two routes that refuse differently calls a legitimate refusal a failure.** Assert the outcome (nothing deleted, grants intact) plus the refusal shape each route actually uses.
- **#138 (TL-1, third instance after #140 M4 and #137): a deny/allow pair between two seeded roles does not pin which action a gate asks.** super_admin holds 54 actions setup_admin lacks; any of them passes. Only a principal holding that ONE action (grant it to the deny fixture, assert it flips to allow) pins the string.
- **#135 (TL-1): a fix found by the dev rather than by a test needs a test before it merges.** `revokedAt` regression was fixed in code and asserted nowhere; deleting the fix left every assertion green.
- **#138 (Dev3): a mutation result is admissible only if the run shows the target test executing.** Harness pointed at the wrong suite printed "20 skipped … SURVIVED"; zero tests executed can neither survive nor kill. Harness must exit SURVIVOR-UNKNOWN when no test ran.
- **#138 (QA-1): a probe that calls past the layer holding the guard reports a hole the product does not have.** Confirm the entry point the product uses (here `applyDirectoryPlan` with `lease`) before reporting a red against a repository call.
- **#138 (QA-1): a mutant classifier that reads a nested `PASS: true` as the verdict manufactures survivors.** Read only the top-level VERDICT; unlisted probes print UNCLASSIFIED, never default green.
...
- **#135 (gate): a quoted list of test paths is ONE jest pattern → "No tests found", exit 1 → reads as a mutant kill.** Pass paths as separate args; a mutant run is admissible only if its `Tests:` line shows tests executed (same class as the #138 false survivor).
- **#138 (Dev3): a route-count pin is a measurement, not arithmetic.** Two branches each wrote 318→319 for their own route; the combined tree mounts both = 320. Write the measured value and say it was measured.
- **#138 (Dev3): jest given a wrong path reports "5 of 6 suites passed" with no error.** A gate that lists suites must assert the suite count it expected (`Test Suites: N passed, N total`), or a skipped check prints PASS.
- **#146 (TL-1): a test can prove a change breaks nothing and still not witness it. Say which of the two a piece of evidence is doing.** doctor.test.js is 46/46 on both postgres images; only the ci.yml guard witnesses the change.
- **#143 (Dev5): a text scanner must blank comments before matching, or prose describing the rule is a false kill on the file that documents it.** Blank with spaces (line numbers stay true), keep a "hook mentioned in a COMMENT is not a hook" control.
- **#141 (TL-1): extending a shape-derived CHECK with a new branch must also tighten the OLD branches** (`appId IS NULL`), or the constraint widens and half-of-each rows become representable again.
- **#138 (gate): a branch older than a merged guard must run the guard from main before claiming green.** #142's hookTimeouts guard merged after 138-combined's base; five new untimed afterAll hooks passed every suite on the branch and failed the guard on main. Gates run main's sweep files against the SHA, not the SHA's copy.
- **#138 (TL-2): a fixture that reads seeded policy tables is not insulated from a seed change by an empty source diff.** Rebuild the DB from the SHA's own schema+seed per run; a cloned template silently lacks the new rows and reads as a code defect.
- **#138 (PMO): run the guard before attributing its output.** PMO grepped `afterAll(async () => {` openers in #138's files and named five offenders; the guard names two other files (both fixed on main by #142, absent from the branch base). Attribution comes from the guard's own output, not a grep for the pattern it checks.
- **#137/#138 (Dev1): a pin belongs on the stack where the row exists.** POLICY_VERSION_ROWS 13 committed on #137's rebase would be red (12 rows there); it lands on the #138 stack where the 13th migration exists. Pin bumps go with the commit that changes the count.
- **#149 (TL-1): deleting a duplicate workflow can silently narrow trigger coverage.** run-tests.yaml fired on every PR; ci.yml only on PRs into approof/main. Widen or record the narrowing in the same commit. Lint helpers with `globals.jest`, do not ignore them — fixtures depend on them being correct.
- **#138 (gate-138c): a seed `update:` branch that no test reaches is unverified.** Every test seeds a fresh DB, so only `create:` runs; the "re-seed heals an existing DB" claim in seed.js:50 has no witness. Add a re-seed-existing-db case or drop the claim.
- **#149 (CI): a test that passes only on a case-insensitive filesystem is untested.** `TextGenWebUiOptions` import resolved on macOS, failed on Linux CI. Import paths must match `git ls-files` byte-for-byte.
- **#149 (CI): a lint step placed before the test step in the same job hides whether tests run.** CI's `test` job runs `lint:ci` first; 1063 pre-existing errors skip jest, and the job's red reads as "tests failed". Lint in its own job.
