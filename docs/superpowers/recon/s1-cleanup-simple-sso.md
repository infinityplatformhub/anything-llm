...
- Ruling round 2 (Techlead pre-check): keep the SSO-forced frontend redirect (retarget `paths.sso.login()` to OIDC start); delete only `SIMPLE_SSO_NO_LOGIN_REDIRECT`, never `SIMPLE_SSO_ENABLED`. Fix `simpleSSOEnabled.js:66` to `isConfirmedSingleUser` inside #50 (supersedes ruling (a)). Remove `sso.issue` permission via new migration slot 090000 (strip from existing key scopes; 020000/045000 untouched). Replace `ssoIssuanceHotfix.test.js` with a `/v1` sweep asserting `mw[0].name === "apiKeyRequired"`; move TemporaryAuthToken single-use HTTP proof to an OIDC callback test. Rebase after #48 (system.js conflict).
