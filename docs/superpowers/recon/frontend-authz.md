...
- Ruling (task 1 review): `WORKSPACE_CAPABILITIES` = exactly the 7 in the approved mockup (workspace.read/write/delete, workspace.members.manage, document.create/delete, chat.send); plan 0b12f76f's 12 were invented (document.update has no gate anywhere; document.search is org-only). Gate-scan tests strip comments/strings and must go red when a gate is commented out; add the workspace-side twin (test 6). Extra gated actions → question in #66.
