...
- Ruling (1b round 2 addendum, Techlead-2): Weaviate classes declare ACL properties as `text` with `tokenization: "field"` (auto-schema `word` tokenization makes `NotEqual` on hyphenated docIds return nothing — silent deny-list failure). 42P01 handling needs a test. pinecone/astra are hosted-only → residual "rendered but unverified", boot report must not say supported. qdrant/chroma get skip-if-unavailable real-store tests like Milvus.
- Ruling (1b round 2 correction, Dev4 measured): a Weaviate class without the ACL properties has no strict fallback either (`Equal orgId` fails like `IsNull`) → refuse the namespace + error log, like Lance legacy tables; `hasAclSchema` also checks tokenization = field.
- Ruling (slice 2 addendum, QA-1 survivors): tests for embed/service `allowedDocumentIds` on pinned docs, `systemActor` with seeded parsed files, and action-specific ACL rows (`document.read` vs `document.search`); no new API — `grantDocumentAcl`/`revokeDocumentAcl` already bump the version; add a staleness test pair (raw vs repository) and a model comment.
- Ruling (slice 2 round 2, Techlead-2 HIGH): `pinnedDocs` and `getContextFiles` enforce the filter's workspaceIds/orgWide/orgId, not the addressed workspace — org-scope chat.send reaches any slug. RED: cross-workspace pinned doc via stream-chat and /v1 → []. embed.js:313 passes the embed actor to resolveProviderConnector.
