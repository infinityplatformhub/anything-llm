...
- Ruling (slice 2 addendum, QA-1 survivors): tests for embed/service `allowedDocumentIds` on pinned docs, `systemActor` with seeded parsed files, and action-specific ACL rows (`document.read` vs `document.search`); add `repository.setDocumentAcl` that bumps policy version — raw prisma ACL writes leave the filter cache stale.
