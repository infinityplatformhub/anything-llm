...
- **เปิดอยู่ 4**: #7 (subagent dev-7, env7 — symlink/temp-name fixes in progress, SHA จะเปลี่ยน), #15 (Dev3 6/12, rebase บน 3c0048b1 เพื่อ spec 09), #22 T-3 (Dev4, t3 `60155631` QA-1 FAIL B1/B2/B3 — non-user actor NaN crash documentFilter:151, cache key ไม่รวม allowedDocumentIds, denied cap — รอ SHA fix), #26 PR-4b (recon `.infi/recon/pr4b.md` 4 file-disjoint PRs — รอ dev ใหม่; 4b ก่อน 4c เสมอ), #25 T-4a route wiring (recon `.infi/recon/t4a-route-wiring.md`, rulings: B-1 grants(creator)∩scopes, W-5 → T-4b, no migration slot — รอ dev ใหม่)
