...
- **main**: e6cea517 — 44/58 closed. #52 MERGED b1dbd4e5 (all 3 verdicts PASS); then Dev2 #58 (4 raw mode readers) → #50 → #53; QA-1 PASS, Techlead PASS, QA-2 PASS scoped + HIGH addendum 7 (isSingleUserMode drift, 3 routes) folded into new SHA (1209/1209; requireSelfSession on /system/user + /web-push; sweep 20 routes; admin.js:313 user.manage; guard formula; 072000 seed WITHDRAWN — org-wide workspace.read matches every workspace; view-as-user reads wait #53). Techlead PASS (nit: enable-multi-user on true single-user needs a test); QA-1 harness + QA-2 single-user probe pending (§7.11); #53 ruling (ข) permissions.scope column in-scope. Then #50 → #53 (Dev2). #30 slice 1a 52b3d176 HOLD (Techlead: flag dead code → ruling (ก) predicate OR-unlabelled; Dev4 new SHA, then rebase 1b); Dev4 → 1b on approof/t5-slice-1b; Dev1 → #48 recon. Queue: #43 #40 after #52; #48 #51 #55 after #41; #49 #54 #56 #57 after #30.
