...
- **main**:  3060171c — 42/52 closed. LIVE HOLE: #52 (impersonated session mutates via engine-less routes; setup_admin cannot grant) — Dev2 hotfix FIRST (final ruling: requirePermission on /system/user + /onboarding, router-stack sweep test, search gate → workspace.read + seed workspace.read to member slot 072000, BASELINE_GRANTABLE=[chat.send] in guard, no setup_admin seed change; QA-1 verdict required before merge §7.11), then #50. QA-2 sweeping all engine-less mutating routes. Dev1 #35, Dev4 #30 slice 1, Dev3 S1 T7-T8. §7.10 resetModules, §7.11 review-is-a-gate added.
