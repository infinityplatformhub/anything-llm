# How to use Dockerized ApproofWorkspace

Use the Dockerized version of ApproofWorkspace for a much faster and complete startup of ApproofWorkspace.

### Minimum Requirements

> [!TIP]
> Running ApproofWorkspace on AWS/GCP/Azure?
...
\*ApproofWorkspace by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*ApproofWorkspace by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized ApproofWorkspace!
...
> the docker container using ApproofWorkspace as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/approofworkspace
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run ApproofWorkspace in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/approofworkspace && \
...
$env:STORAGE_LOCATION="$HOME\Documents\approofworkspace"; `
...
services:
  approofworkspace:
    image: mintplexlabs/approofworkspace
    container_name: approofworkspace
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - approofworkspace_storage:/app/server/storage
    restart: always

volumes:
  approofworkspace_storage:
    driver: local
    driver_opts:
      type: none
      o: bind
...
Go to `http://localhost:3001` and you are now using ApproofWorkspace! All your data and progress will persist between
...
The integrations below are templates or tooling built by the community to make running the docker experience of ApproofWorkspace easier.

### Use the Midori AI Subsystem to Manage ApproofWorkspace

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the ApproofWorkspace docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in ApproofWorkspace.
